...
export async function requestCredentialFromIssuer(didUri: any, customerName: any, country: any) {
...
export function renderCredential(credentialJwt: string) {
  const vc: Partial<VcDataModel> = Jwt.parse({ jwt: credentialJwt }).decoded.payload['vc'] as Partial<VcDataModel>
  return {
    title: (vc.type as any)[(vc.type as any).length - 1].replace(/(?<!^)(?<![A-Z])[A-Z](?=[a-z])/g, ' $&'), // get the last credential type in the array and format it with spaces
